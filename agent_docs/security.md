...
### Authorization Bypass in Payout Admin Functions (2026-01-09)

**Severity:** HIGH
**Status:** OPEN
**Files Affected:**
- `lib/api/payouts.ts`
- `features/finances/actions/payoutActions.ts`

#### Description

The admin functions `approvePayout()`, `rejectPayout()`, and `markPayoutPaid()` accept user IDs as parameters but perform **NO authorization validation** to verify that the calling user has admin/internal privileges. These functions use the admin Supabase client which bypasses RLS.

The Server Actions that call these functions only verify authentication (user is logged in), NOT authorization (user has admin role).

#### Vulnerable Pattern

```typescript
// In payoutActions.ts - MISSING AUTHORIZATION CHECK
export async function approvePayoutAction(id: string) {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();

  if (!user) {  // ← Only checks authentication!
...
1. Developer (non-admin) is authenticated
2. Developer calls `approvePayoutAction()` directly from browser console or custom script
3. Server Action passes auth check (user is logged in)
4. No role verification occurs
5. Developer approves their own payout, bypassing the approval workflow

#### Recommended Fix

Add role-based authorization checks in Server Actions:
...
Or use the existing `requireRole()` guard:

```typescript
import { requireRole } from '@/lib/auth/guards';

export async function approvePayoutAction(id: string) {
  const profile = await requireRole(['admin', 'internal']);
  const result = await approvePayout(id, profile.id);
  // ...
}
```
...
The payouts table has RLS policies restricting UPDATE to admin/internal roles. However, `lib/api/payouts.ts` uses the **admin Supabase client** (service role key), which bypasses ALL RLS policies. This makes application-level authorization checks mandatory.

#### Cleanup Status

**2026-01-14:** Duplicate files (`payouts 2.ts`, `payouts 3.ts`) no longer exist. Only `payouts.ts` and `payouts.shared.ts` remain.
