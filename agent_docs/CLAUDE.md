...
- **Gameplan Tab** — Rich text documents with @mentions, version history (admin/internal/dev only)
- **Dev Logging System** — Daily check-ins with position tracking, snooze, 24h edit lock
- **Delay Tracking** — Client/dev delays with progress timeline adjustment
- **Extension Requests** — DFY approval workflow for deadline extensions
...
- `project_documents` - Rich text docs for Gameplan tab (Plate.js)
- `document_versions` - Version history with checkpoints
- `dev_checkins` - Daily dev check-ins (progress/no_work/delay)
- `checkin_notes` - Per-deliverable notes with position changes
- `project_delays` - Client/dev delay tracking
- `project_extensions` - Deadline extension requests with DFY approval
