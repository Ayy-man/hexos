...
import { ExportPDFButtonWrapper as ExportPDFButton } from '@/features/inquiries/components/ExportPDFButtonWrapper'
...
const FIELD_LABELS: Record<string, string> = {
  ...
  meeting_recording_url: 'Meeting Recording',
}

// Enum value to display label mappings
const ENUM_VALUE_LABELS: Record<string, Record<string, string>> = {
  ...
  budget_indication: {
    ...
    general_range: 'Vague range',
    no_budget: 'No mention',
  },
  engagement_level: {
  ...
  relationship_type: {
    warm_referral: 'Warm referral or existing client',
    warm_outreach: 'Good vibe from discovery call',
    cold_lead: 'Cold lead / first conversation',
  },
  contact_role: {
    founder: 'Founder / Decision-maker',
...
// Helper to get display value for enum fields
function getEnumLabel(field: string, value: string): string {
  const fieldLabels = ENUM_VALUE_LABELS[field]
  if (!fieldLabels) return value
...
export default async function InquiryDetailPage({
  ...
  try {
    ...
    console.error('[Inquiry Page Error]', {
      inquiryId: id,
  ...
  // Also mark inquiry as viewed by admin (sets admin_viewed_at)
  const isAdminRole = ['admin', 'internal'].includes(profile.role)
  if (isAdminRole) {
    try {
      const { updateInquiryStage, markInquiryAsViewed } = await import('@/lib/api/inquiries')
      // Mark as viewed (idempotent - only sets if not already set)
      await markInquiryAsViewed(id)
      // Auto-advance from unopened if needed
      if (inquiry.proposal_stage === 'unopened') {
        await updateInquiryStage(id, 'admin_reviewed', 'Auto-advanced on first admin view')
        inquiry.proposal_stage = 'admin_reviewed'
      }
    } catch (error) {
      console.warn('Failed to update inquiry view status:', error)
    }
  }
                    ...
                    {Object.entries(formData)
                      ...
                      .map(([key, value]) => {
                        ...
                        if (Array.isArray(value)) {
                          ...
                          // Check if this is an enum field and get human-readable label
                        ...
                        const isUrlField = key.includes('url') || key.includes('website') || key.includes('link')
                        const isValidUrl = isUrlField && displayValue.startsWith('http')
