...
import type { PulseDailyTask, PulseEvent, PulseStats } from '@/lib/types/pulse';
...
export interface CachedPulseData {
  user_id: string;
  tasks_by_date: Record<string, PulseDailyTask[]>; // ISO date string -> tasks
  events: PulseEvent[]; // Last 30 days
  stats: PulseStats;
...
export async function cachePulseData(
  userId: string,
  tasksByDate: Record<string, PulseDailyTask[]>,
  events: PulseEvent[],
  stats: PulseStats,
