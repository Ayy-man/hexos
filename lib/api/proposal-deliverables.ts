...
export interface ProposalDeliverableWithChildren extends ProposalDeliverable {
  children: ProposalDeliverableWithChildren[]
}
...
export { buildDeliverableTree, flattenDeliverableTree } from './proposal-deliverables.utils'

// Get deliverables as tree structure
export async function getProposalDeliverablesTree(
  inquiryId: string
): Promise<ProposalDeliverableWithChildren[]> {
  const flat = await getProposalDeliverables(inquiryId)
  // Import dynamically to avoid circular dependency
  const { buildDeliverableTree } = await import('./proposal-deliverables.utils')
  return buildDeliverableTree(flat)
}
...
export async function updateProposalDeliverable(
  ...
  // Parent relationship (for hierarchy changes)
