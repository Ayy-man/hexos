...
import { Briefcase, Star, Eye, EyeOff, Filter, Send } from 'lucide-react'
...
interface DevOpportunitiesContentProps {
  ...
  bidOpportunityIds: string[]
  devId: string
}

export function DevOpportunitiesContent({
  opportunities,
  bidOpportunityIds,
  devId,
}: DevOpportunitiesContentProps) {
  ...
  const [bidOpportunity, setBidOpportunity] = useState<OpportunityWithPrefs | null>(null)

  // Track bids submitted this session (for optimistic update)
  const [newBidIds, setNewBidIds] = useState<Set<string>>(new Set())

  // Combined bid IDs (server + session)
  const allBidOpportunityIds = useMemo(() => {
    return new Set([...bidOpportunityIds, ...newBidIds])
  }, [bidOpportunityIds, newBidIds])
  ...
  // Open bid dialog for an opportunity
  const handleOpenBidDialog = (opportunity: OpportunityWithPrefs) => {
    setBidOpportunity(opportunity)
  ...
  const handleBidSuccess = () => {
    if (bidOpportunity) {
      setNewBidIds(prev => new Set([...prev, bidOpportunity.id]))
    }
      ...
      {filteredOpportunities.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {filteredOpportunities.map((opportunity) => {
            const hasBid = allBidOpportunityIds.has(opportunity.id)
            return (
                ...
                {!hasBid && (
                  <div className="absolute bottom-2 right-2">
                    <Button
                      ...
                      onClick={(e) => {
                        e.stopPropagation()
                        handleOpenBidDialog(opportunity)
                      }}
                    >
          ...
          {bidOpportunity && (
            <Tabs defaultValue="bid" className="w-full">
              ...
              <TabsContent value="precommit" className="mt-4">
                <PreCommitmentTab
                  opportunityId={bidOpportunity.id}
                  currentStatus={bidOpportunity.commitment_status || null}
                  currentNote={bidOpportunity.commitment_note || null}
                  committedAt={bidOpportunity.committed_at || null}
                  onUpdate={() => {
