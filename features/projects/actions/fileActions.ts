...
import type { FileView, ProjectFileItem } from '@/lib/api/project-files.shared'
import {
  createFolder,
  createDocument,
  renameItem,
...
/**
 * Get all files for a project (server action)
 * Returns files based on user role visibility
 */
export async function getProjectFilesAction(
  projectId: string
): Promise<{ files: ProjectFileItem[]; error?: string }> {
  try {
    ...
    const files = await getProjectFileTree(projectId)
    return { files }
  } catch (err) {
    console.error('[Action] Error fetching files:', err)
