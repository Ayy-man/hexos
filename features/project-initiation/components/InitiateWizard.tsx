...
import type { ProposalDeliverableWithChildren } from '@/lib/api/proposal-deliverables'
import { flattenDeliverableTree } from '@/lib/api/proposal-deliverables.utils'
...
// Helper to get all IDs from tree
function getAllDeliverableIds(items: ProposalDeliverableWithChildren[]): string[] {
  const ids: string[] = []
...
export function InitiateWizard({ inquiry, deliverables: initialDeliverables, templates }: InitiateWizardProps) {
  ...
  // Step 1: Deliverables (as tree structure, can be updated via server actions)
  const [deliverables, setDeliverables] = useState(initialDeliverables)
  const [selectedDeliverableIds, setSelectedDeliverableIds] = useState<string[]>(
    getAllDeliverableIds(initialDeliverables) // Select all by default
  )
  ...
  // Validation - deliverables step always valid (can add manually or skip)
  const isStepValid = (step: StepId): boolean => {
    switch (step) {
      case 'deliverables':
        return true // Allow proceeding even with no deliverables - user can add manually or skip
      case 'requirements':
  ...
  // Calculate total price from selected deliverables (flatten tree first)
  const flatDeliverables = flattenDeliverableTree(deliverables)
  const selectedDeliverables = flatDeliverables.filter(d => selectedDeliverableIds.includes(d.id))
