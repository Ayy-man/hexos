...
const ENUM_LABELS: Record<string, Record<string, string>> = {
  ...
  build_preference: {
    quick_win: 'Quick Win (24-48 hour proposal)',
    full_build: 'Full Build (48-72 hour proposal)',
  },
  problem_importance: {
...
function getEnumDisplayValue(field: string, value: unknown): string | null {
  if (typeof value !== 'string') return null
  const fieldLabels = ENUM_LABELS[field]
  ...
  for (const [sectionName, fields] of Object.entries(FIELD_SECTIONS)) {
    ...
    for (const field of sectionFields) {
      ...
      if (Array.isArray(value)) {
        // Filter out non-primitive items and join
        displayValue = value
          .filter((item): item is string | number => typeof item === 'string' || typeof item === 'number')
          .map(String)
          .join(', ')
      } else if (typeof value === 'object' && value !== null) {
        // Skip objects - they shouldn't be displayed directly
        continue
      } else {
        // Check if this is an enum field and get human-readable label
        const enumLabel = getEnumDisplayValue(field, value)
        displayValue = enumLabel || String(value)
      }
