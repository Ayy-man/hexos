  ...
  const handleCopyFromProposal = useCallback(async () => {
    startTransition(async () => {
      try {
        ...
        // Update editor content with proposal content instead of page reload
        if (proposalContent && Array.isArray(proposalContent) && proposalContent.length > 0) {
          editor.tf.setValue(proposalContent)
          setHasChanges(false)
          setLastSaved(new Date())
        }
        toast.success('Proposal imported to your version')
      } catch (error) {
        console.error('Failed to import proposal:', error)
        toast.error('Failed to import proposal')
      }
    })
  }, [onCopyFromProposal, proposalContent, editor])
          ...
          <div className="flex items-center gap-2">
            ...
            {hasProposalContent && (
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    <AlertDialogTitle>Import from Proposal?</AlertDialogTitle>
                    <AlertDialogDescription>
                      {hasExistingContent
                        ? 'This will replace your current version with the official proposal. Any changes you\'ve made will be lost.'
                        : 'This will import the official proposal to your version. You can then customize it for your client.'}
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
