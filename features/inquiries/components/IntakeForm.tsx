...
export function IntakeForm({ blueprints, caseStudies, partnerName }: IntakeFormProps) {
  ...
  const handleBack = () => {
    ...
    if (step === 'closed_type' || step === 'proposal_type') {
      // Clear deal type selections when going back to initial to avoid stale state
      setValue('closed_deal_type', undefined)
      setValue('proposal_type', undefined)
      setStep('initial')
    } else if (step === 'path_form') {
      if (data.submission_type === 'closed') {
        setStep('closed_type')
  ...
  const handleSetField = (fieldName: string, value: unknown) => {
    ...
    setValue(fieldName as any, value, {
      shouldDirty: true,
      shouldTouch: true,
      shouldValidate: true, // Trigger validation to ensure re-render
    })
    ...
    requestAnimationFrame(() => {
      try {
        ...
        // Look for elements with name attribute, data-field attribute, or id
        const selectors = [
          `[name="${escapedName}"]`,
  ...
  return (
    <FormProvider {...methods}>
      <div className="flex flex-col lg:flex-row gap-6">
        {/* Form Panel */}
        <div className={`flex-1 ${showCopilot ? 'lg:max-w-[60%]' : 'max-w-3xl mx-auto'}`}>
          <Card>
            <CardHeader>
            ...
            <CardContent>
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                {step === 'initial' && <InitialStep onNext={handleNext} />}
                {step === 'closed_type' && <ClosedDealType onNext={handleNext} />}
                {step === 'proposal_type' && (
                  currentPath === 'B1' ? (
                    <BlueprintInfo
                      onBack={handleBack}
                      onCustomize={() => {
                        // Switch to variation path and continue
                        setValue('proposal_type', 'variation')
                        setStep('path_form')
                      }}
                    />
                  ) : (
                    <ProposalType onNext={handleNext} />
                  )
                )}
